<div>
    <div wire:ignore class="dropdown d-inline-block">
        <button @click="$wire.readNotifications()" type="button" class="btn header-item noti-icon"
            id="page-header-notifications-dropdown" data-bs-toggle="dropdown" role="button" aria-haspopup="true"
            aria-expanded="false">
                ...
                @foreach ($notifications->take(5) as $notification)
                    @if ($notification->type == 'failed_submissions')
                        <a href="{!! $notification->data['link'] ?? null !!}" class="text-reset notification-item">
                            <div class="p-3 rounded d-flex align-items-start">
                                ...
                                <div class="flex-grow-1">
                                    <h6 class="mb-1">Submissions Notification!
                    ...
                    @if ($notification->type == 'submissions')
                        <a href="{!! $notification->data['link'] ?? null !!}" class="text-reset notification-item">
                            <div class="p-3 rounded d-flex align-items-start">
                                ...
                                <div class="flex-grow-1">
                                    <h6 class="mb-1">Submission Notification!
                                        @if (is_null($notification->read_at))
                                            <span class="badge bg-warning">New!</span>
                                        @endif
                                    </h6>
                    ...
                    @if ($notification->type == 'imports')
                        <a href="{!! $notification->data['link'] ?? null !!}" class="text-reset notification-item">
                            <div class="p-3 rounded d-flex align-items-start">
                                ...
                                <div class="flex-grow-1">
                                    <h6 class="mb-1">Import Successful!
                                        @if (is_null($notification->read_at))
                                            <span class="badge bg-warning">New!</span>
                                        @endif
                                    </h6>
                    ...
                    @if ($notification->type == 'failed_imports')
                        <a href="{!! $notification->data['link'] ?? null !!}" class="text-reset notification-item">
                            <div class="p-3 rounded d-flex align-items-start">
                                ...
                                <div class="flex-grow-1">
                                    <h6 class="mb-1">Failed to Import your file!
                                        @if (is_null($notification->read_at))
                                            <span class="badge bg-warning">New!</span>
                                        @endif
                                    </h6>
                                    <div class="text-muted">
                                        <p class="mb-1 font-size-13">Your file failed to import. Please try again.</p>
                                        <p class="mb-0 font-size-10 text-uppercase fw-bold">
                                            <i class="mdi mdi-clock-outline"></i>
                                            {{ \Carbon\Carbon::parse($notification->created_at)->diffForHumans() }}
                                        </p>
