<div class="table-responsive pb-5 col-md-12" style="margin: 10px 0 10px;" wire:ignore x-data="{ show: $wire.entangle('loadingData') }"
    ...
    <table class="table table-striped  nowrap align-middle w-100" id="rpmagree">
    ...
    <script>
        $(document).ready(function() {
            ...
            $wire.on('loaded-data-farmer', (e) => {
                populateFarmerTable(e.agreement);
            });

            function populateFarmerTable(data) {

                if ($.fn.DataTable.isDataTable('#rpmagree')) {
                    $('#rpmagree').DataTable().clear().destroy();
                }


                let tbody = $('#rpmagree tbody');
                ...
                data.forEach(function(row) {
                    ...
                    tr.append($('<td>').text(row.partner_name));
