...
final class RpmFarmerBasic extends PowerGridComponent
{
    public function datasource(): Collection
    {
        $changedArray = collect();
        $data = RtcProductionFarmer::query()->whereNotNull('area_under_basic_seed_multiplication')
            ->lazy() // Lazy loading for memory efficiency
            ->each(function ($item) use (&$changedArray) {
                $names = json_decode($item->area_under_basic_seed_multiplication) ?? [];
