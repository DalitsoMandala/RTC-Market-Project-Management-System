...
class indicator_2_3_3
{
    ...
    public function getDisaggregations()
    {
        ...
        $total = $this->builderFarmer()->where('is_registered_seed_producer', true)
            ->where('group', 'Seed multiplier')->count();
        return [
            'Total' => $total,
            'Cassava' => $this->getCrop()['cassava'],
            'Potato' => $this->getCrop()['potato'],
            'Sweet potato' => $this->getCrop()['sweet_potato'],
            'Domestic markets' => $this->getMarketType()['total_doms'],
            'International markets' => $this->getMarketType()['total_intermarkets'],
            'Individual farmers not in POs' => $this->getActorType()['individual'],
            'POs' => $this->getActorType()['pos'],
